(function () {
  ...
  function friendlyRuntimeMessage(error, fallbackMessage) {
    ...
    if (
      fallbackMessage &&
      !/^erro inesperado em tempo de execução\\.?$/i.test(String(fallbackMessage).trim()) &&
      !/^falha assíncrona não tratada\\.?$/i.test(String(fallbackMessage).trim())
    ) {
  ...
  function emit(level, message, error, context) {
    const resolvedMessage =
      level === 'error' ? friendlyRuntimeMessage(error, message) : safeString(message);
    const payload = {
      level,
      message: safeString(resolvedMessage),
      error: error?.message || error || null,
      context: context || null,
      at: new Date().toISOString()
    };
    try {
      console[level === 'error' ? 'error' : 'warn']('[Synapse]', payload);
    } catch (_) {}
    ...
    if (level === 'error') {
      try {
        window.dispatchEvent(new CustomEvent('synapse:error', { detail: payload }));
      } catch (_) {}
    }
  }
  let busyCount = 0;
    ...
    start(label) {
      ...
      try {
        window.dispatchEvent(
          new CustomEvent('synapse:busy', {
            detail: { busy: true, label: label || 'Processando...' }
          })
        );
      } catch (_) {}
    },
      ...
      try {
        window.dispatchEvent(new CustomEvent('synapse:busy', { detail: { busy: busyCount > 0 } }));
      } catch (_) {}
    }
  };
  ...
  window.addEventListener('error', event =>
    emit('error', 'Erro inesperado em tempo de execução.', event.error || event.message, {
  ...
  window.addEventListener('unhandledrejection', event =>
    emit('error', 'Falha assíncrona não tratada.', event.reason)
  );
})();
(function () {
...
(function () {
  ...
  const storage = {
    async get(key) {
      try {
        const value = persistentStorage ? persistentStorage.getItem(key) : memoryStorage[key];
        return value == null ? null : { key, value };
      } catch (error) {
        window.SynapseLogger?.warn('Falha ao ler armazenamento local.', error, { key });
        return memoryStorage[key] == null ? null : { key, value: memoryStorage[key] };
      }
    },
    async set(key, value) {
      ...
      try {
        ...
        window.SynapseLogger?.warn('Falha ao persistir armazenamento local.', error, { key });
      }
      return { key, value };
    }
  };
  function safeSessionGet(key) {
    try {
      return sessionStorageSafe ? sessionStorageSafe.getItem(key) : null;
    } catch (_) {
      return null;
    }
  }
...
(function () {
  ...
  function readFileAsArrayBuffer(file) {
    return new Promise((resolve, reject) => {
      ...
      if (file.size > MAX_FILE_BYTES)
        return reject(new Error('O arquivo excede o limite de 15 MB.'));
  ...
  async function parse(file) {
    ...
    try {
      ...
      if (!headers.length)
        throw new Error('Não foi possível encontrar cabeçalhos na primeira linha.');
      ...
      sample.forEach(row => {
        ...
        marker.textContent = JSON.stringify(row);
