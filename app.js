...
function AuthScreen() {
 ...
 const googleButton = mode === 'login' ? React.createElement('button',{type:'button',className:'auth-google',onClick:signInGoogle,disabled:busy},
  React.createElement('span',{className:'google-logo','aria-hidden':'true'},
   React.createElement('svg',{viewBox:'0 0 18 18',width:'18',height:'18'},
    React.createElement('path',{fill:'#4285F4',d:'M17.64 9.205c0-.638-.057-1.252-.164-1.841H9v3.482h4.844a4.14 4.14 0 0 1-1.797 2.716v2.258h2.908c1.702-1.567 2.685-3.878 2.685-6.615Z'}),
    React.createElement('path',{fill:'#34A853',d:'M9 18c2.43 0 4.467-.806 5.955-2.18l-2.908-2.258c-.806.54-1.834.859-3.047.859-2.344 0-4.33-1.584-5.04-3.713H.955v2.332A9 9 0 0 0 9 18Z'}),
    React.createElement('path',{fill:'#FBBC05',d:'M3.96 10.708A5.41 5.41 0 0 1 3.675 9c0-.593.102-1.17.285-1.708V4.96H.955A9 9 0 0 0 0 9c0 1.453.348 2.827.955 4.04l3.005-2.332Z'}),
    React.createElement('path',{fill:'#EA4335',d:'M9 3.58c1.321 0 2.507.454 3.44 1.346l2.582-2.582C13.463.891 11.425 0 9 0A9 9 0 0 0 .955 4.96L3.96 7.292C4.67 5.163 6.656 3.58 9 3.58Z'})
   )
  ),
  React.createElement('span',null,'Continuar com Google')
 ) : null;
 const authDivider = mode === 'login' ? React.createElement('div',{className:'auth-divider'},React.createElement('span',null,'ou')) : null;
...
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme, user, onLogout }) {
 ...
 const userKey = user?.id ? 'synapse-profile-avatar-' + user.id : 'synapse-profile-avatar';
 const [avatar, setAvatar] = useState(() => { try { return user?.user_metadata?.avatar_url || persistentStorage?.getItem(userKey) || ''; } catch(e) { return user?.user_metadata?.avatar_url || ''; } });
 useEffect(() => { try { setAvatar(user?.user_metadata?.avatar_url || persistentStorage?.getItem(userKey) || ''); } catch(e) {} }, [user?.id, user?.user_metadata?.avatar_url]);
 const handleProfilePhoto = (e) => {
  const file = e.target.files?.[0]; e.target.value=''; if(!file || !file.type.startsWith('image/')) return;
  const reader = new FileReader();
  reader.onload = () => { const value = String(reader.result || ''); setAvatar(value); try { persistentStorage?.setItem(userKey, value); } catch(err) {} };
  reader.readAsDataURL(file);
 };
 const initials = (user?.user_metadata?.full_name || user?.email || 'S').trim().slice(0,1).toUpperCase();
 const avatarNode = avatar ? React.createElement('img',{src:avatar,alt:'Foto de perfil',className:'account-avatar account-avatar-image'}) : React.createElement('span',{className:'account-avatar'},initials);
 const items = [
 ...
 const accountMenu = React.createElement('div',{className:'account-menu'},
  React.createElement('div',{className:'account-menu-head'},avatarNode,React.createElement('div',{className:'min-w-0'},React.createElement('b',null,user?.user_metadata?.full_name || 'Conta pessoal'),React.createElement('small',null,user?.email || ''))),
  React.createElement('button',{onClick:()=>profileInputRef.current?.click(),className:'account-menu-item'},'Alterar foto'),
  React.createElement('button',{onClick:()=>setAccountOpen(false),className:'account-menu-item'},'Fechar menu'),
  React.createElement('button',{onClick:onLogout,className:'account-menu-item danger'},'Sair da conta')
 );
 return React.createElement('aside',{className:'synapse-sidebar'},
  React.createElement('div',{className:'sidebar-brand'},React.createElement('div',{className:'brand-mark'},'S'),React.createElement('div',{className:'sidebar-brand-copy'},React.createElement('b',null,'Synapse'),React.createElement('small',null,'espaço comercial')),React.createElement('div',{className:'mobile-account-wrap'},React.createElement('button',{className:'mobile-account-button',onClick:()=>setAccountOpen(v=>!v),title:'Minha conta'},avatarNode,React.createElement(ChevronDown,{size:15})),accountOpen && accountMenu)),
  ...
  React.createElement('div',{className:'sidebar-bottom'},React.createElement('div',{className:'sidebar-tools'},React.createElement('div',{className:'flex items-center gap-1.5 text-sm',style:{color:streak>0?'var(--ember)':'var(--muted)'}},React.createElement(Flame,{size:16,strokeWidth:2}),React.createElement('span',null,streak,' ',streak===1?'dia':'dias')),backupButtons),React.createElement('div',{className:'account-name account-trigger'},React.createElement('button',{onClick:()=>setAccountOpen(v=>!v),className:'account-trigger-button'},avatarNode,React.createElement('div',{className:'account-trigger-copy'},React.createElement('small',null,'Conta pessoal'),React.createElement('b',null,user?.email||'Synapse')),React.createElement(ChevronDown,{size:14})),accountOpen && accountMenu)),
  React.createElement('input',{ref:profileInputRef,type:'file',accept:'image/*',className:'hidden',onChange:handleProfilePhoto})
 );
}
