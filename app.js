...
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme, user, onLogout }) {
 ...
 return React.createElement('aside',{className:'synapse-sidebar'},
  ...
  React.createElement('div',{className:'sidebar-bottom'},React.createElement('div',{className:'sidebar-tools'},React.createElement('div',{className:'flex items-center gap-1.5 text-sm',style:{color:streak>0?'var(--ember)':'var(--muted)'}},React.createElement(Flame,{size:16,strokeWidth:2}),React.createElement('span',null,streak,' ',streak===1?'dia':'dias')),backupButtons),React.createElement('div',{className:'account-wrap'},
  React.createElement('button',{className:'account-trigger',onClick:()=>setAccountOpen(v=>!v),title:'Abrir menu da conta'},
   React.createElement('span',{className:'account-avatar'},(user?.email||'S').slice(0,1).toUpperCase()),
   React.createElement('span',{className:'account-name-text'},React.createElement('small',null,'Conta pessoal'),React.createElement('b',null,user?.email||'Synapse')),
   React.createElement(ChevronDown,{size:14,className:accountOpen?'account-chevron open':'account-chevron'})
  ),
  accountOpen && React.createElement('div',{className:'account-menu'},
   React.createElement('div',{className:'account-menu-head'},React.createElement('span',{className:'account-avatar account-menu-avatar'},(user?.email||'S').slice(0,1).toUpperCase()),React.createElement('div',null,React.createElement('strong',null,'Conta pessoal'),React.createElement('span',null,user?.email||'Synapse'))),
   React.createElement('button',{className:'account-menu-logout',onClick:()=>{setAccountOpen(false);onLogout();}},'Sair da conta')
  )
 ))
 );
}
