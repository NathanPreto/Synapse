...
function AuthScreen() {
 ...
 return React.createElement('div',{className:'auth-screen'},
  React.createElement('div',{className:'auth-card'},
   React.createElement('div',{className:'auth-brand'},React.createElement('img',{className:'auth-logo',src:'./synapse-mark.png',alt:'Synapse'}),React.createElement('div',null,React.createElement('div',{className:'auth-title'},'Synapse'),React.createElement('div',{className:'auth-subtitle'},'Gestão comercial e mentalidade'))),
   ...
   mode === 'login' && React.createElement('div',{className:'auth-social'},
    React.createElement('button',{type:'button',onClick:signInGoogle,disabled:busy,className:'auth-google'},
     React.createElement('span',{className:'google-logo','aria-hidden':'true'},
      React.createElement('svg',{viewBox:'0 0 24 24',width:20,height:20},
       React.createElement('path',{fill:'#4285F4',d:'M21.35 12.23c0-.78-.07-1.53-.22-2.25H12v4.26h5.24a4.48 4.48 0 0 1-1.94 2.94v2.45h3.14c1.84-1.69 2.91-4.18 2.91-7.4Z'}),
       React.createElement('path',{fill:'#34A853',d:'M12 21.5c2.63 0 4.84-.87 6.45-2.36l-3.14-2.45c-.87.58-1.98.92-3.31.92-2.54 0-4.69-1.72-5.46-4.03H3.3v2.53A9.74 9.74 0 0 0 12 21.5Z'}),
       React.createElement('path',{fill:'#FBBC05',d:'M6.54 13.58A5.85 5.85 0 0 1 6.24 12c0-.55.1-1.08.3-1.58V7.89H3.3A9.48 9.48 0 0 0 2.25 12c0 1.48.35 2.88 1.05 4.11l3.24-2.53Z'}),
       React.createElement('path',{fill:'#EA4335',d:'M12 6.39c1.43 0 2.71.49 3.72 1.46l2.79-2.79C16.83 3.51 14.63 2.5 12 2.5a9.74 9.74 0 0 0-8.7 5.39l3.24 2.53C7.31 8.11 9.46 6.39 12 6.39Z'})
      )
     ),
...
function App() {
 ...
 useEffect(() => {
  ...
  supabaseClient.auth.getSession().then(({data,error}) => {
   if (!active) return;
   if (error) { console.error('Falha ao recuperar sessão:', error); setSession(null); }
  ...
  const { data: listener } = supabaseClient.auth.onAuthStateChange((_event, nextSession) => { if(active) setSession(nextSession || null); });
...
function AuthLoading(){ return React.createElement('div',{className:'auth-screen'},React.createElement('div',{className:'auth-card auth-loading'},React.createElement('img',{className:'auth-logo',src:'./synapse-mark.png',alt:'Synapse'}),React.createElement('div',{className:'auth-title'},'Synapse'),React.createElement('div',{className:'auth-copy'},'Carregando seu espaço...'))); }

function formatSupabaseError(error) {
 if (!error) return 'Erro desconhecido';
 return [error.message, error.code ? `código ${error.code}` : '', error.details ? `detalhes: ${error.details}` : '', error.hint ? `dica: ${error.hint}` : ''].filter(Boolean).join(' | ');
}
...
function SynapseWorkspace({ user, onLogout }) {
 ...
 const [theme, setTheme] = useState(() => { try { return persistentStorage?.getItem('mental-vendas-theme') || 'light'; } catch (e) { return 'dark'; } });
 ...
 const syncQueues = React.useRef({});
 const queueSync = (key, task) => {
  const previous = syncQueues.current[key] || Promise.resolve();
  const next = previous.catch(()=>{}).then(task);
  syncQueues.current[key] = next.catch(()=>{});
  return next;
 };
 useEffect(() => {
  let active = true;
  ...
  (async () => {
   try {
    ...
    if(active) { setLoadError(''); setLoaded(true); }
   } catch(e) {
    console.error('Falha ao carregar Synapse:', e, { code:e?.code, details:e?.details, hint:e?.hint, status:e?.status });
    if (active) {
 ...
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueSync('clients',()=>syncUserRows('clients',user.id,clients.map(c=>({id:c.id,name:c.name,contact:c.contact||'',stage:c.stage||'novo',temp:c.temp||'morno',last_contact:c.lastContact||null,created_at_date:c.createdAt||todayStr(),notes:c.notes||'',lost_reason:c.lostReason||'',lost_tags:c.lostTags||[],closed_at:c.closedAt||null})))).catch(e=>{console.error('Sincronização de clientes falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível sincronizar clientes. Seus dados locais foram preservados.');}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[clients,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueSync('reminders',()=>syncUserRows('reminders',user.id,reminders.map(r=>({id:r.id,text:r.text,due:r.due||null,client_id:r.clientId||null,done:!!r.done})))).catch(e=>{console.error('Sincronização de lembretes falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível sincronizar lembretes. Seus dados locais foram preservados.');}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[reminders,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueSync('checkins',()=>syncUserRows('checkins',user.id,checkins.map(c=>({id:c.id,date:c.date,mood:c.mood,identity:c.identity||'',note:c.note||'',reframe:c.reframe||'',mental_stages:c.mentalStages||{}})))).catch(e=>{console.error('Sincronização de check-ins falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível sincronizar check-ins. Seus dados locais foram preservados.');}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[checkins,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueSync('settings',()=>syncSettings(user.id,{entries:desidentificationEntries,pinned:pinnedPhrase,templates,theme})).catch(e=>{console.error('Sincronização de configurações falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível sincronizar configurações. Seus dados locais foram preservados.');}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[desidentificationEntries,pinnedPhrase,templates,theme,loaded,user.id]);
 ...
 function deleteCheckin(id) {
  ...
  deleteCloudRow('checkins', user.id, id).then(()=>setCheckins(prev => prev.filter(c => c.id !== id))).catch(e=>{console.error('Exclusão de check-in falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível excluir o check-in da nuvem. O registro foi preservado.');});
 }
 ...
 deleteCloudRow('clients', user.id, id).then(()=>setClients(prev => prev.filter(c => c.id !== id))).catch(e=>{console.error('Exclusão de cliente falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível excluir o cliente da nuvem. O registro foi preservado.');});
 }
 ...
 function removeReminder(id) { deleteCloudRow('reminders', user.id, id).then(()=>setReminders(prev => prev.filter(r => r.id !== id))).catch(e=>{console.error('Exclusão de lembrete falhou:',e,{code:e?.code,details:e?.details,hint:e?.hint,status:e?.status});setSyncError('Não foi possível excluir o lembrete da nuvem. O registro foi preservado.');}); }
 ...
 if (!loaded) {
  return React.createElement(Shell, { theme },
   React.createElement('div', { className: 'p-10 text-center', style: { color: 'var(--muted)', maxWidth:'720px', margin:'0 auto' } },
    loadError
     ? React.createElement(React.Fragment,null,
       React.createElement('div',{style:{color:'var(--rose)',fontWeight:800,marginBottom:'8px'}},'Não foi possível carregar seus dados.'),
       React.createElement('div',{style:{lineHeight:1.6,marginBottom:'16px'}},loadError),
       React.createElement('button',{className:'header-primary',onClick:()=>setLoadAttempt(v=>v+1)},'Tentar novamente')
      )
...
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme, user, onLogout }) {
 ...
 return React.createElement('aside',{className:'synapse-sidebar'},
  React.createElement('div',{className:'sidebar-brand'},React.createElement('img',{className:'brand-mark brand-mark-image',src:'./synapse-mark.png',alt:'Synapse'}),React.createElement('div',null,React.createElement('b',null,'Synapse'),React.createElement('small',null,'espaço comercial'))),
