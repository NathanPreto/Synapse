...
function App() {
 ...
 const [aiKey, setAiKey] = useState(() => safeSessionGet('mental-vendas-gemini-key') || '');
 ...
 if (c1)
 setCheckins(JSON.parse(c1.value));
 if (c2)
 setClients(JSON.parse(c2.value));
 if (c3)
 setReminders(JSON.parse(c3.value));
 if (c4) {
  const parsed = JSON.parse(c4.value);
  setDesidentificationEntries(Array.isArray(parsed?.entries) ? parsed.entries : Array.isArray(parsed) ? parsed : []);
  setPinnedPhrase(parsed?.pinned || null);
 }
 if (c5) setTemplates(Array.isArray(JSON.parse(c5.value)) ? JSON.parse(c5.value) : []);
 }
 catch (e) {
 console.error(e);
 }
 ...
 useEffect(() => { if (loaded)
 storage.set('mindset-checkins', JSON.stringify(checkins)).catch(console.error); }, [checkins, loaded]);
 useEffect(() => { if (loaded)
 storage.set('mindset-clients', JSON.stringify(clients)).catch(console.error); }, [clients, loaded]);
 useEffect(() => { if (loaded)
 storage.set('mindset-reminders', JSON.stringify(reminders)).catch(console.error); }, [reminders, loaded]);
 useEffect(() => { if (loaded) storage.set('mental-vendas-desidentificacao', JSON.stringify({ entries: desidentificationEntries, pinned: pinnedPhrase })).catch(console.error); }, [desidentificationEntries, pinnedPhrase, loaded]);
 useEffect(() => { if (loaded) storage.set('mental-vendas-templates', JSON.stringify(templates)).catch(console.error); }, [templates, loaded]);
 ...
 a.href = url;
 a.download = `assumido-backup-${todayStr()}.json`;
 ...
 return React.createElement(Shell, { theme },
  React.createElement(TopBar, { streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme }),
 ...
 return React.createElement('div',{className:'modal-backdrop'},
  React.createElement('div',{className:'smart-modal ai-modal'},
   React.createElement('div',{className:'flex items-start justify-between gap-4'},
    React.createElement('div',null,
     React.createElement('div',{className:'flex items-center gap-2 text-lg font-semibold'},React.createElement(Sparkles,{size:18}),' Assistente Mental Vendas'),
...
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme }) {
 ...
 React.createElement("div", null,
 React.createElement("div", { className: "serif text-xl", style: { fontWeight: 600 } }, "Mental Vendas"),
