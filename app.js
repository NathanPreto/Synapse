...
// Serializa gravações por tabela para evitar que duas alterações rápidas
// terminem fora de ordem no banco. O estado do React continua sendo a fonte
// de verdade da interface; o Supabase é a fonte de verdade persistente.
const cloudSyncQueues = Object.create(null);
function queueCloudSync(key, task) {
  const previous = cloudSyncQueues[key] || Promise.resolve();
  const next = previous.catch(() => {}).then(task);
  cloudSyncQueues[key] = next.catch(() => {});
  return next;
}
...
function clearLegacyLocalData() {
  const keys = [
    ...
    'mental-vendas-theme',
    'synapse-legacy-migration-skipped'
  ];
  keys.forEach(key => { try { persistentStorage?.removeItem(key); } catch (e) {} });
}
 ...
 const submit = async (e) => {
  ...
  try {
   ...
   if (mode === 'signup') {
    ...
    const { error } = await supabaseClient.auth.resetPasswordForEmail(email.trim(), { redirectTo: window.location.origin + window.location.pathname });
    if (error) throw error;
...
function App() {
 ...
 if (!supabaseClient) return React.createElement(AuthConfigError, null);
...
function AuthConfigError(){ return React.createElement('div',{className:'auth-screen'},React.createElement('div',{className:'auth-card'},React.createElement('div',{className:'auth-brand'},React.createElement('div',{className:'auth-logo'},'S'),React.createElement('div',null,React.createElement('div',{className:'auth-title'},'Synapse'),React.createElement('div',{className:'auth-subtitle'},'Configuração necessária'))),React.createElement('div',{className:'auth-heading'},'Não foi possível conectar ao Supabase'),React.createElement('p',{className:'auth-copy'},'Verifique se config.js contém a URL do projeto e a publishable key corretas e se a biblioteca do Supabase foi carregada antes do app.js.'),React.createElement('button',{className:'auth-submit',onClick:()=>window.location.reload()},'Tentar novamente'))); }

async function syncUserRows(table, userId, rows) {
 if (!supabaseClient || !userId) return;
 const safeRows = Array.isArray(rows) ? rows : [];
 const payload = safeRows.map(r => ({ ...r, id: String(r.id), user_id: userId }));
 return queueCloudSync(`rows:${userId}:${table}`, async () => {
  const { data: existing, error: readError } = await supabaseClient.from(table).select('id').eq('user_id', userId);
  if (readError) throw readError;
  const ids = new Set(payload.map(r => String(r.id)));
  const stale = (existing || []).map(r => String(r.id)).filter(id => !ids.has(id));
  if (stale.length) {
   const { error } = await supabaseClient.from(table).delete().eq('user_id', userId).in('id', stale);
   if (error) throw error;
  }
...
async function syncSettings(userId, settings) {
 ...
 const payload = {
  user_id: userId,
  id: userId,
  desidentification_entries: Array.isArray(settings.entries) ? settings.entries : [],
  pinned_phrase: settings.pinned || null,
  templates: Array.isArray(settings.templates) ? settings.templates : [],
  theme: settings.theme === 'light' ? 'light' : 'dark'
 };
 return queueCloudSync(`settings:${userId}`, async () => {
  const { error } = await supabaseClient.from('app_settings').upsert(payload, { onConflict: 'user_id' });
  if (error) throw error;
 });
}
...
function SynapseWorkspace({ user, onLogout }) {
  ...
  (async () => {
   try {
    ...
    if (cloud.hasCloudData) {
     ...
     const alreadySkipped = (() => { try { return persistentStorage?.getItem('synapse-legacy-migration-skipped') === '1'; } catch (e) { return false; } })();
     if (!alreadySkipped) {
      ...
      const safeParse = (item, fallback) => {
       ...
       try { return JSON.parse(item.value); } catch (e) { return fallback; }
      };
      const localClients=safeParse(c2,[]); const localReminders=safeParse(c3,[]); const localCheckins=safeParse(c1,[]);
      const localMental=safeParse(c4,{}); const localTemplates=safeParse(c5,[]);
      if(localClients.length||localReminders.length||localCheckins.length||localTemplates.length||localMental.entries?.length||localMental.pinned){
       setLocalMigrationData({clients:localClients,reminders:localReminders,checkins:localCheckins,entries:Array.isArray(localMental.entries)?localMental.entries:[],pinned:localMental.pinned||null,templates:Array.isArray(localTemplates)?localTemplates:[]});
 ...
 useEffect(()=>{ if(!loaded)return; syncUserRows('clients',user.id,clients.map(c=>({id:c.id,name:c.name,contact:c.contact||'',stage:c.stage||'novo',temp:c.temp||'morno',last_contact:c.lastContact||null,created_at_date:c.createdAt||todayStr(),notes:c.notes||'',lost_reason:c.lostReason||'',lost_tags:c.lostTags||[],closed_at:c.closedAt||null}))).catch(e=>{console.error(e);setSyncError('Não foi possível sincronizar clientes.');}); },[clients,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; syncUserRows('reminders',user.id,reminders.map(r=>({id:r.id,text:r.text,due:r.due||null,client_id:r.clientId||null,done:!!r.done}))).catch(e=>{console.error(e);setSyncError('Não foi possível sincronizar lembretes.');}); },[reminders,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; syncUserRows('checkins',user.id,checkins.map(c=>({id:c.id,date:c.date,mood:c.mood,identity:c.identity||'',note:c.note||'',reframe:c.reframe||'',mental_stages:c.mentalStages||{}}))).catch(e=>{console.error(e);setSyncError('Não foi possível sincronizar check-ins.');}); },[checkins,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; syncSettings(user.id,{entries:desidentificationEntries,pinned:pinnedPhrase,templates,theme}).catch(e=>{console.error(e);setSyncError('Não foi possível sincronizar configurações.');}); },[desidentificationEntries,pinnedPhrase,templates,theme,loaded,user.id]);
 ...
 async function migrateLocalData(){
  ...
  } catch(e){console.error(e);setSyncError('Não foi possível importar os dados locais para a nuvem.');alert('Não foi possível concluir a importação. Nenhum dado local foi apagado.');}
 }
 ...
 return React.createElement(Shell, { theme },
  ...
  React.createElement('div', { className: 'px-4 sm:px-6 lg:px-8 py-6 w-full max-w-none' },
   ...
   tab === 'lembretes' && React.createElement(Lembretes, { followUps, reminders, addReminder, toggleReminder, removeReminder, markContacted: (id) => updateClient(id, { lastContact: todayStr() }), clients }),
   tab === 'foco' && React.createElement(FocoDoDia, { items: dailyFocus, reminders, toggleReminder, goTo: setTab, clients }),
  ),
  migrationRequested && React.createElement(MigrationModal,{onImport:migrateLocalData,onSkip:()=>{ try { persistentStorage?.setItem('synapse-legacy-migration-skipped','1'); } catch(e) {} setMigrationRequested(false); setLocalMigrationData(null); }}),
...
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme, user, onLogout }) {
 ...
 React.createElement("div", { className: "max-w-3xl mx-auto w-full px-5 md:px-8 pt-6 flex items-center justify-between" },
 React.createElement("div", null,
 React.createElement("div", { className: "serif text-xl", style: { fontWeight: 600 } }, "Synapse"),
 React.createElement("div", { className: "text-xs", style: { color: 'var(--muted)' } }, "mentalidade + clientes + a\u00E7\u00E3o, no mesmo registro")),
 React.createElement("div", { className: "flex items-center gap-3" },
 React.createElement("div", { className: "flex items-center gap-1.5 text-sm", style: { color: streak > 0 ? 'var(--ember)' : 'var(--muted)' } },
 React.createElement(Flame, { size: 16, strokeWidth: 2 }),
 React.createElement("span", null,
 streak,
 " ",
 streak === 1 ? 'dia' : 'dias')),
 React.createElement("div", { className: "flex items-center gap-1 pl-2", style: { borderLeft: '1px solid var(--border)' } },
 React.createElement("button", { onClick: exportBackup, title: "Baixar backup (.json)", className: "p-1.5 rounded", style: { color: 'var(--muted)' } },
 React.createElement(Download, { size: 15 })),
 React.createElement("button", { onClick: () => { var _a; return (_a = fileInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }, title: "Importar backup (.json)", className: "p-1.5 rounded", style: { color: 'var(--muted)' } },
 React.createElement(Upload, { size: 15 })),
 React.createElement("button", { onClick: () => { var _a; return (_a = excelInputRef.current) === null || _a === void 0 ? void 0 : _a.click(); }, title: "Importar clientes do Excel", className: "p-1.5 rounded", style: { color: 'var(--teal)' } },
 React.createElement(Users, { size: 15 })),
 React.createElement("button", { onClick: toggleTheme, title: theme === 'dark' ? "Ativar tema claro" : "Ativar tema escuro", className: "p-1.5 rounded", style: { color: 'var(--muted)', fontSize: 16, lineHeight: 1 } }, theme === 'dark' ? "☀" : "◐"),
 React.createElement("input", { ref: fileInputRef, type: "file", accept: "application/json", className: "hidden", onChange: (e) => { var _a; const f = (_a = e.target.files) === null || _a === void 0 ? void 0 : _a[0]; if (f)
 importBackup(f); e.target.value = ''; } }),
 React.createElement("input", { ref: excelInputRef, type: "file", accept: ".xlsx,.xls,.csv,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,text/csv", className: "hidden", onChange: (e) => { var _a; const f = (_a = e.target.files) === null || _a === void 0 ? void 0 : _a[0]; if (f)
 importExcel(f); e.target.value = ''; } })))),
 React.createElement("div", { className: "max-w-3xl mx-auto w-full px-5 md:px-8 flex gap-5 mt-5 overflow-x-auto" }, items.map(({ key, label, IconC }) => (React.createElement("button", { key: key, onClick: () => setTab(key), className: "flex items-center gap-1.5 pb-3 text-sm whitespace-nowrap", style: {
 color: tab === key ? 'var(--text)' : 'var(--muted)',
 borderBottom: tab === key ? '2px solid var(--ember)' : '2px solid transparent',
 } },
 React.createElement(IconC, { size: 15 }),
 " ",
 label)))),React.createElement('div',{className:'top-account'},React.createElement('span',{className:'top-account-email'},user?.email||''),React.createElement('button',{onClick:onLogout,className:'top-logout'},'Sair'))));
}
...
function Mental({ checkins, todayCheckin, saveCheckin, deleteCheckin, justSaved, desidentificationEntries, setDesidentificationEntries, pinnedPhrase, setPinnedPhrase }) {
 ...
 const [answers, setAnswers] = useState(() => {
  const saved = todayCheckin && todayCheckin.mentalStages;
  if (saved && typeof saved === 'object') return saved;
  return { pensar: '', nomear: '', perceber: '', questionar: '', desidentificar: '', distancia: '' };
 });
 const [mood, setMood] = useState((todayCheckin === null || todayCheckin === void 0 ? void 0 : todayCheckin.mood) || 3);
 const [identity, setIdentity] = useState((todayCheckin === null || todayCheckin === void 0 ? void 0 : todayCheckin.identity) || '');
 const [note, setNote] = useState((todayCheckin === null || todayCheckin === void 0 ? void 0 : todayCheckin.note) || '');
 const [reframe, setReframe] = useState((todayCheckin === null || todayCheckin === void 0 ? void 0 : todayCheckin.reframe) || '');
...
function ClientCard({ client, expanded, onToggle, updateClient, removeClient, templates = [] }) {
 ...
 return (React.createElement("div", { className: "p-2.5 rounded text-sm", style: { background: 'var(--surface)', border: '1px solid var(--border)' } },
 React.createElement("div", { className: "flex items-center justify-between cursor-pointer", onClick: onToggle },
 React.createElement("div", null,
 ...
 expanded && (React.createElement("div", { className: "mt-3 space-y-2 pt-3", style: { borderTop: '1px solid var(--border)' } },
 ...
 React.createElement("button", { onClick: () => updateClient(client.id, { lastContact: todayStr() }), className: "text-xs flex items-center gap-1", style: { color: 'var(--teal)' } },
 React.createElement(Clock, { size: 12 }),
 " marcar contato hoje"),
 React.createElement("button", { onClick: () => removeClient(client.id), style: { color: 'var(--rose)' } },
...
function Lembretes({ followUps, reminders, addReminder, toggleReminder, removeReminder, markContacted }) {
 ...
 React.createElement("button", { onClick: () => toggleReminder(r.id), className: "w-4 h-4 rounded-full shrink-0", style: { border: '1.5px solid var(--muted)' } }),
 ...
 React.createElement("button", { onClick: () => removeReminder(r.id), style: { color: 'var(--muted)' } },
 ...
 React.createElement("button", { onClick: () => removeReminder(r.id) },
