...
function friendlyAuthError(err) {
 ...
 if (lower.includes('email not confirmed')) return 'Confirme seu e-mail antes de entrar no Synapse.';
 if (lower.includes('user already registered')) return 'Este e-mail já possui uma conta. Tente entrar ou recuperar a senha.';
 if (lower.includes('password should be at least')) return 'A senha precisa ter pelo menos 8 caracteres.';
...
function PasswordRules({ password }) {
 const r = passwordRules(password);
 const items = [['length','8+ caracteres'],['upper','1 letra maiúscula'],['lower','1 letra minúscula'],['number','1 número'],['symbol','1 símbolo']];
 return React.createElement('div',{className:'password-rules'},
  items.map(([key,label]) => React.createElement('div',{key,className:r[key]?'password-rule ok':'password-rule'},React.createElement('span',{className:'password-rule-dot'},r[key]?'✓':'•'),label))
 );
}
function GoogleMark() {
 return React.createElement('span',{className:'google-mark','aria-hidden':'true'},
  React.createElement('svg',{viewBox:'0 0 48 48',focusable:'false'},
   React.createElement('path',{fill:'#EA4335',d:'M24 9.5c3.54 0 6.7 1.22 9.19 3.61l6.85-6.85C35.9 2.38 30.47 0 24 0 14.61 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.09 17.74 9.5 24 9.5z'}),
   React.createElement('path',{fill:'#4285F4',d:'M46.98 24.55c0-1.64-.15-3.22-.42-4.73H24v9.02h12.9c-.56 3-2.26 5.54-4.82 7.24l7.79 6.05c4.54-4.19 7.11-10.36 7.11-17.58z'}),
   React.createElement('path',{fill:'#FBBC05',d:'M10.54 28.59A14.5 14.5 0 0 1 9.5 24c0-1.59.27-3.14.76-4.59l-7.98-6.19A23.98 23.98 0 0 0 0 24c0 3.87.93 7.53 2.58 10.78l7.96-6.19z'}),
   React.createElement('path',{fill:'#34A853',d:'M24 48c6.48 0 11.93-2.13 15.9-5.92l-7.79-6.05c-2.16 1.45-4.92 2.3-8.11 2.3-6.26 0-11.57-3.59-13.46-8.91l-7.98 6.19C6.51 42.62 14.61 48 24 48z'})
  )
 );
}
function AuthBrand() {
...
function AuthScreen() {
 ...
 const signInGoogle = async () => {
  ...
  try {
   ...
   setError(friendlyAuthError(err)); setBusy(false);
  }
 };
 const title = mode === 'signup' ? 'Crie sua conta' : mode === 'reset' ? 'Recupere seu acesso' : 'Bem-vindo de volta';
  ...
  React.createElement('section',{className:'auth-shell'},
   ...
   React.createElement('div',{className:'auth-card'},
    ...
    mode === 'login' && React.createElement(React.Fragment,null,
     React.createElement('button',{type:'button',className:'auth-google',onClick:signInGoogle,disabled:busy},React.createElement(GoogleMark,null),React.createElement('span',null,'Continuar com Google')),
     React.createElement('div',{className:'auth-divider'},React.createElement('span',null,'ou'))
    ),
    React.createElement('form',{onSubmit:submit,className:'auth-form'},
     ...
     mode !== 'reset' && React.createElement('label',null,React.createElement('span',null,'Senha'),React.createElement('input',{type:'password',value:password,onChange:e=>setPassword(e.target.value),placeholder:'••••••••',required:true,minLength:8,autoComplete:mode==='signup'?'new-password':'current-password'})),
     mode === 'signup' && React.createElement(PasswordRules,{password}),
     mode === 'signup' && React.createElement('label',null,React.createElement('span',null,'Confirmar senha'),React.createElement('input',{type:'password',value:confirmPassword,onChange:e=>setConfirmPassword(e.target.value),placeholder:'Repita sua senha',required:true,minLength:8,autoComplete:'new-password'})),
    ...
    React.createElement('div',{className:'auth-links'},
     mode === 'login' && React.createElement('button',{type:'button',onClick:()=>switchMode('signup')},'Criar uma conta'),
     mode === 'login' && React.createElement('button',{type:'button',onClick:()=>switchMode('reset')},'Esqueci minha senha'),
     mode !== 'login' && React.createElement('button',{type:'button',onClick:()=>switchMode('login')},'Voltar para entrar')
    ),
    React.createElement('div',{className:'auth-note'},'Seus dados ficam separados por conta e protegidos no banco.')
   )
  )
 );
}
...
function PasswordRecoveryScreen({ onDone }) {
 ...
 return React.createElement('main',{className:'auth-screen'},React.createElement('div',{className:'auth-card auth-card-single'},
  React.createElement(AuthBrand,null),
  React.createElement('div',{className:'auth-heading'},'Criar nova senha'),
  React.createElement('p',{className:'auth-copy'},'Escolha uma nova senha para recuperar o acesso à sua conta.'),
  React.createElement('form',{onSubmit:submit,className:'auth-form'},
   React.createElement('label',null,React.createElement('span',null,'Nova senha'),React.createElement('input',{type:'password',value:password,onChange:e=>setPassword(e.target.value),required:true,minLength:8,autoComplete:'new-password'})),
   React.createElement(PasswordRules,{password}),
   React.createElement('label',null,React.createElement('span',null,'Confirmar nova senha'),React.createElement('input',{type:'password',value:confirmPassword,onChange:e=>setConfirmPassword(e.target.value),required:true,minLength:8,autoComplete:'new-password'})),
   error && React.createElement('div',{className:'auth-alert error'},error),
   message && React.createElement('div',{className:'auth-alert success'},message),
   React.createElement('button',{type:'submit',disabled:busy,className:'auth-submit'},busy?'Aguarde...':'Atualizar senha')
  )
 ));
}
...
function SynapseWorkspace({ user, onLogout }) {
 ...
 useEffect(()=>{if(!loaded)return;persistence.queueSettings(user.id,{entries:desidentificationEntries,pinned:pinnedPhrase,templates,theme}).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar configurações.',e);setSyncError('Não foi possível preparar a sincronização das configurações.');});},[desidentificationEntries,pinnedPhrase,templates,theme,loaded,user.id]);
 ...
 async function migrateLocalData(){
  ...
  } catch(e){window.SynapseLogger?.warn('Falha na migração local.', e);alert('Não foi possível concluir a importação. Verifique sua conexão e tente novamente.');}
 }
 ...
 const lossReasons = useMemo(() => { const counts={}; clients.forEach(c => { (Array.isArray(c.lostTags)?c.lostTags:[]).forEach(t => { const label=String(t||'').trim(); if(label && LOST_TAGS.includes(label)) counts[label]=(counts[label]||0)+1; }); }); return Object.entries(counts).sort((a,b)=>b[1]-a[1]); }, [clients]);
 ...
 function deleteCheckin(id){if(!id)return;if(!confirm('Excluir este autorreconhecimento do histórico? Esta ação não pode ser desfeita.'))return;setCheckins(prev=>prev.filter(c=>c.id!==id));persistence.queueDelete(user.id,'checkins',id).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar exclusão.',e);setSyncError('O check-in foi removido da tela e será sincronizado quando possível.');});}
 function saveCheckin(mood,identity,note,reframe,mentalStages){identity=sanitizeInput(identity);note=sanitizeInput(note);reframe=sanitizeInput(reframe);const existing=checkins.find(c=>c.date===todayStr());const next={...(existing||{}),id:(existing&&existing.id)||uid(),date:todayStr(),mood,identity,note,reframe,mentalStages:mentalStages||(existing&&existing.mentalStages)||{}};setCheckins(prev=>[...prev.filter(c=>c.date!==todayStr()),next]);persistence.queueUpsert(user.id,'checkins',next).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar check-in.',e);setSyncError('O check-in foi salvo localmente e será sincronizado quando possível.');});setJustSaved(true);setTimeout(()=>setJustSaved(false),1800);}
 function addClient(name,contact){name=sanitizeInput(name,200);contact=sanitizeInput(contact,500);const client={id:uid(),name,contact,stage:'novo',temp:'morno',lastContact:todayStr(),createdAt:todayStr(),notes:'',lostReason:'',lostTags:[],closedAt:null};setClients(prev=>[...prev,client]);persistence.queueUpsert(user.id,'clients',client).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar cliente.',e);setSyncError('O cliente foi salvo localmente e será sincronizado quando possível.');});}
 function updateClient(id,patch){const cleanPatch=sanitizeRecord(patch,['name','contact','notes','lostReason']);if(Array.isArray(patch.lostTags))cleanPatch.lostTags=patch.lostTags.map(tag=>sanitizeInput(tag,100));setClients(prev=>{const current=prev.find(c=>c.id===id);if(!current)return prev;const next={...current,...cleanPatch};if(patch.stage==='fechado'&&!current.closedAt)next.closedAt=todayStr();if(patch.stage&&patch.stage!=='fechado')next.closedAt=null;persistence.queueUpsert(user.id,'clients',next).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar cliente.',e);setSyncError('A alteração foi salva localmente e será sincronizada quando possível.');});return prev.map(c=>c.id===id?next:c);});}
 function removeClient(id){const client=clients.find(c=>c.id===id),label=client?'"'+client.name+'"':'este cliente';if(!confirm('Excluir '+label+'? Esta ação não pode ser desfeita.'))return;setClients(prev=>prev.filter(c=>c.id!==id));persistence.queueDelete(user.id,'clients',id).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar exclusão.',e);setSyncError('O cliente foi removido da tela e a exclusão será sincronizada quando possível.');});}
 function addReminder(text,due,clientId){text=sanitizeInput(text,1000);const reminder={id:uid(),text,due,clientId:clientId||null,done:false};setReminders(prev=>[...prev,reminder]);persistence.queueUpsert(user.id,'reminders',reminder).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar lembrete.',e);setSyncError('O lembrete foi salvo localmente e será sincronizado quando possível.');});}
 function toggleReminder(id){setReminders(prev=>{const current=prev.find(r=>r.id===id);if(!current)return prev;const next={...current,done:!current.done};persistence.queueUpsert(user.id,'reminders',next).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar lembrete.',e);setSyncError('A alteração foi salva localmente e será sincronizada quando possível.');});return prev.map(r=>r.id===id?next:r);});}
 function updateReminder(id,patch){patch=sanitizeRecord(patch,['text']);setReminders(prev=>{const current=prev.find(r=>r.id===id);if(!current)return prev;const next={...current,...patch};persistence.queueUpsert(user.id,'reminders',next).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar lembrete.',e);setSyncError('A alteração foi salva localmente e será sincronizada quando possível.');});return prev.map(r=>r.id===id?next:r);});}
 function removeReminder(id){setReminders(prev=>prev.filter(r=>r.id!==id));persistence.queueDelete(user.id,'reminders',id).catch(e=>{window.SynapseLogger?.warn('Falha ao enfileirar exclusão.',e);setSyncError('O lembrete foi removido da tela e a exclusão será sincronizada quando possível.');});}
 ...
 SynapseSpreadsheet.parse(file).then(({rows, headers}) => {
  try {
   ...
   window.SynapseLogger?.warn('Falha ao preparar a planilha para importação.', err, { fileName:file?.name });
 ...
 function getSynLocalAnswer(question) {
  ...
  if (/o que (voce|vc|tu) (pode|consegue) fazer|como voce pode me ajudar|como vc pode me ajudar|o que voce faz|o que vc faz/.test(normalized)) return 'Posso te ajudar a usar o Synapse, organizar clientes e próximos passos, criar lembretes, explicar as áreas do app e dar dicas simples de organização comercial e vendas.';
  if (/dica|dicas|organizacao comercial|organizar minhas vendas|organizar as vendas/.test(normalized)) return 'Aqui vão 3 dicas práticas:\n1. Registre sempre o próximo passo de cada cliente.\n2. Separe o que precisa de ação hoje do que pode esperar.\n3. Termine o dia revisando os follow-ups para não deixar oportunidades paradas.';
  if (/indisponivel|indisponível|por que.*parou|porque.*parou|por que.*nao respondeu|porque.*nao respondeu/.test(normalized)) return 'Quando a Syn fica indisponível, normalmente é porque o serviço de resposta atingiu um limite temporário ou demorou para responder. Seus dados do Synapse continuam separados e salvos; você pode tentar novamente em alguns instantes.';
 ...
 if (loadError) {
  return React.createElement(Shell, { theme },
   React.createElement('div',{className:'cloud-load-state'},
    React.createElement('div',{className:'cloud-load-card'},
     ...
     React.createElement('button',{onClick:()=>{setLoadError('');setLoaded(false);setLoadNonce(v=>v+1)},className:'cloud-load-retry'},'Tentar novamente')
    )
   )
  );
 }
...
function WorkspaceHeader({tab,setTab,theme,toggleTheme,user,onLogout}) {
 ...
 const name = user?.user_metadata?.full_name || user?.user_metadata?.name || user?.email || 'Conta pessoal';
 const avatar = user?.user_metadata?.avatar_url || user?.user_metadata?.picture || '';
 return React.createElement('header',{className:'workspace-header'},
  React.createElement('div',null,React.createElement('div',{className:'workspace-eyebrow'},now),React.createElement('h1',null,labels[tab]||labels.painel)),
  React.createElement('div',{className:'workspace-actions'},
   React.createElement('button',{onClick:toggleTheme,className:'header-icon',title:theme==='dark'?'Ativar tema claro':'Ativar tema escuro'},theme==='dark'?'☀':'◐'),
   React.createElement('button',{onClick:()=>setTab('clientes'),className:'header-primary'},'+ Novo cliente'),
   React.createElement('div',{className:'header-account'},
    React.createElement('button',{onClick:()=>setOpen(v=>!v),className:'header-account-trigger',title:'Abrir menu da conta','aria-expanded':open},avatar?React.createElement('img',{src:avatar,alt:'',referrerPolicy:'no-referrer'}):React.createElement('span',null,(name||'S').slice(0,1).toUpperCase())),
    open && React.createElement('div',{className:'header-account-menu'},
     React.createElement('div',{className:'header-account-user'},
  ...
  React.createElement(Section, { title: 'Autorreconhecimento emocional' },
   React.createElement('div', { className: 'p-4 rounded space-y-4', style: { background: 'var(--surface)', border: '1px solid var(--border)' } },
    ...
    React.createElement('div', { className: 'pt-2' },
     ...
     React.createElement('textarea', { value: text, onChange: e => updateAnswer(e.target.value), rows: 4, className: 'w-full p-3 rounded text-sm', placeholder: current.placeholder }),
  ...
  React.createElement(Section, { title: 'Contexto do check-in' },
   React.createElement('div', { className: 'p-4 rounded space-y-4', style: { background: 'var(--surface)', border: '1px solid var(--border)' } },
    React.createElement('div', null,
     ...
     React.createElement('div', { className: 'mood-selector' }, MOODS.map(m => React.createElement('button', { key: m.v, onClick: () => setMood(m.v), className: 'py-2 rounded text-xs', style: { background: mood === m.v ? m.color : 'var(--surface2)', color: mood === m.v ? 'var(--on-accent)' : 'var(--muted)', border: '1px solid var(--border)' } }, m.label)))),
