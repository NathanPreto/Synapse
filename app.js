...
function AuthScreen() {
 ...
 const signInGoogle = async () => {
  ...
  try {
   ...
   if (error) throw error;
  } catch (err) { setError(err?.message || 'Não foi possível entrar com o Google.'); setBusy(false); }
 };
 ...
 return React.createElement('div',{className:'auth-screen'},
  React.createElement('div',{className:'auth-card'},
   ...
   mode === 'login' && React.createElement('div',{className:'auth-social'},
    React.createElement('button',{type:'button',onClick:signInGoogle,disabled:busy,className:'auth-google'},
     React.createElement('span',{className:'google-mark','aria-hidden':'true'},React.createElement('span',null,'G')),
...
function App() {
 ...
 useEffect(() => {
  ...
  supabaseClient.auth.getSession().then(({data,error}) => { if(!active)return; if(error){ console.error('Falha ao recuperar sessão:',error); setSession(null); } else { setSession(data.session); } setAuthLoading(false); }).catch(error => { if(active){ console.error('Falha ao recuperar sessão:',error); setSession(null); setAuthLoading(false); } });
...
function formatSupabaseError(error) {
 ...
 return parts.join(' — ') || 'Erro desconhecido na nuvem.';
}
...
const syncQueues = Object.create(null);
function queueCloudSync(table, task) {
 if (!syncQueues[table]) syncQueues[table] = Promise.resolve();
 syncQueues[table] = syncQueues[table].catch(()=>{}).then(task);
 return syncQueues[table];
}

async function replaceCloudRows(table, userId, rows) {
 if (!supabaseClient || !userId) return;
 const safeRows = Array.isArray(rows) ? rows : [];
 ...
 if (!safeRows.length) return;
 const payload = safeRows.map(r => ({...r, id:String(r.id), user_id:userId}));
 const { error: upsertError } = await supabaseClient.from(table).upsert(payload, { onConflict:'id' });
...
function SynapseWorkspace({ user, onLogout }) {
 ...
 useEffect(() => {
  ...
  (async () => {
   try {
    ...
    console.error('Falha ao carregar Synapse:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});
    if(active) { setLoadError(`Não foi possível carregar seus dados da nuvem. ${formatSupabaseError(e)}`); setLoaded(false); }
    return;
   }
 ...
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueCloudSync('clients',()=>syncUserRows('clients',user.id,clients.map(c=>({id:c.id,name:c.name,contact:c.contact||'',stage:c.stage||'novo',temp:c.temp||'morno',last_contact:c.lastContact||null,created_at_date:c.createdAt||todayStr(),notes:c.notes||'',lost_reason:c.lostReason||'',lost_tags:c.lostTags||[],closed_at:c.closedAt||null})))).then(()=>setSyncError('')).catch(e=>{console.error('Falha ao sincronizar clientes:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});setSyncError(`Não foi possível sincronizar clientes. ${formatSupabaseError(e)}`);}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[clients,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueCloudSync('reminders',()=>syncUserRows('reminders',user.id,reminders.map(r=>({id:r.id,text:r.text,due:r.due||null,client_id:r.clientId||null,done:!!r.done})))).then(()=>setSyncError('')).catch(e=>{console.error('Falha ao sincronizar lembretes:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});setSyncError(`Não foi possível sincronizar lembretes. ${formatSupabaseError(e)}`);}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[reminders,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueCloudSync('checkins',()=>syncUserRows('checkins',user.id,checkins.map(c=>({id:c.id,date:c.date,mood:c.mood,identity:c.identity||'',note:c.note||'',reframe:c.reframe||'',mental_stages:c.mentalStages||{}})))).then(()=>setSyncError('')).catch(e=>{console.error('Falha ao sincronizar check-ins:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});setSyncError(`Não foi possível sincronizar check-ins. ${formatSupabaseError(e)}`);}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[checkins,loaded,user.id]);
 useEffect(()=>{ if(!loaded)return; const t=setTimeout(()=>{ queueCloudSync('settings',()=>syncSettings(user.id,{entries:desidentificationEntries,pinned:pinnedPhrase,templates,theme})).then(()=>setSyncError('')).catch(e=>{console.error('Falha ao sincronizar configurações:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});setSyncError(`Não foi possível sincronizar configurações. ${formatSupabaseError(e)}`);}); },SYNC_DEBOUNCE_MS); return ()=>clearTimeout(t); },[desidentificationEntries,pinnedPhrase,templates,theme,loaded,user.id]);
 ...
 async function retryCloudLoad(){
  ...
  try {
   ...
   if (cloud.hasCloudData) {
    ...
    setClients([]); setReminders([]); setCheckins([]); setDesidentificationEntries([]); setPinnedPhrase(null); setTemplates([]);
   }
   setLoaded(true);
  } catch(e) {
   console.error('Falha ao recarregar Synapse:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint});
   setLoadError(`Não foi possível carregar seus dados da nuvem. ${formatSupabaseError(e)}`);
  }
 }
 ...
 function deleteCheckin(id) {
  ...
  (async()=>{
   try { await deleteCloudRow('checkins',user.id,id); setCheckins(prev => prev.filter(c => c.id !== id)); setSyncError(''); }
   catch(e){ console.error('Falha ao excluir check-in:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint}); setSyncError(`Não foi possível excluir o check-in. ${formatSupabaseError(e)}`); }
  })();
 }
 ...
 async function removeReminder(id) {
  if (!id) return;
  try { await deleteCloudRow('reminders',user.id,id); setReminders(prev => prev.filter(r => r.id !== id)); setSyncError(''); }
  catch(e){ console.error('Falha ao excluir lembrete:',e,{message:e?.message,code:e?.code,details:e?.details,hint:e?.hint}); setSyncError(`Não foi possível excluir o lembrete. ${formatSupabaseError(e)}`); }
 }
 ...
 return;
 const nextCheckins = Array.isArray(data.checkins) ? data.checkins : [];
 const nextClients = Array.isArray(data.clients) ? data.clients : [];
 const nextReminders = Array.isArray(data.reminders) ? data.reminders : [];
 const nextEntries = Array.isArray(data.desidentificationEntries) ? data.desidentificationEntries : [];
 const nextPinned = data.pinnedPhrase || null;
 const nextTemplates = Array.isArray(data.templates) ? data.templates : [];
 (async()=>{
  try {
   await Promise.all([
    replaceCloudRows('clients',user.id,nextClients.map(c=>({id:c.id,name:c.name,contact:c.contact||'',stage:c.stage||'novo',temp:c.temp||'morno',last_contact:c.lastContact||null,created_at_date:c.createdAt||todayStr(),notes:c.notes||'',lost_reason:c.lostReason||'',lost_tags:c.lostTags||[],closed_at:c.closedAt||null}))),
    replaceCloudRows('reminders',user.id,nextReminders.map(r=>({id:r.id,text:r.text,due:r.due||null,client_id:r.clientId||null,done:!!r.done}))),
    replaceCloudRows('checkins',user.id,nextCheckins.map(c=>({id:c.id,date:c.date,mood:c.mood,identity:c.identity||'',note:c.note||'',reframe:c.reframe||'',mental_stages:c.mentalStages||{}}))),
    syncSettings(user.id,{entries:nextEntries,pinned:nextPinned,templates:nextTemplates,theme})
   ]);
   setCheckins(nextCheckins); setClients(nextClients); setReminders(nextReminders); setDesidentificationEntries(nextEntries); setPinnedPhrase(nextPinned); setTemplates(nextTemplates); setSyncError('');
  } catch(err){ console.error('Falha ao importar backup:',err,{message:err?.message,code:err?.code,details:err?.details,hint:err?.hint}); setSyncError(`Não foi possível importar o backup. ${formatSupabaseError(err)}`); }
 })();
 }
 catch (err) {
 ...
 if (!loaded) {
  return React.createElement(Shell, { theme },
   React.createElement('div',{className:'cloud-load-screen'},
    React.createElement('div',{className:'cloud-load-card'},
     React.createElement('div',{className:'cloud-load-title'},loadError?'Não foi possível carregar seus dados':'Carregando seu espaço...'),
     React.createElement('div',{className:'cloud-load-copy'},loadError||'Aguarde enquanto o Synapse recupera seus dados da nuvem.'),
     loadError && React.createElement('button',{className:'cloud-load-retry',onClick:retryCloudLoad},'Tentar novamente')
    )
   )
  );
 }
...
function AccountMenu({ user, onLogout }) {
 ...
 const [avatar,setAvatar] = useState(() => { try { return persistentStorage?.getItem('synapse-avatar-'+user.id) || ''; } catch(e){ return ''; } });
 const inputRef = React.useRef(null);
 const displayName = user?.user_metadata?.full_name || user?.email?.split('@')[0] || 'Conta pessoal';
 const googleAvatar = user?.user_metadata?.avatar_url || user?.user_metadata?.picture || '';
 const shownAvatar = avatar || googleAvatar;
 const initials = (displayName || 'S').slice(0,1).toUpperCase();
 useEffect(()=>{
  const close = e => { if (!e.target.closest('.account-menu-wrap')) setOpen(false); };
 ...
 const changeAvatar = e => {
  ...
  const reader=new FileReader();
  reader.onload=()=>{ const value=String(reader.result||''); setAvatar(value); try{persistentStorage?.setItem('synapse-avatar-'+user.id,value);}catch(err){} };
  reader.readAsDataURL(file);
  e.target.value='';
 };
 return React.createElement('div',{className:'account-menu-wrap'},
  React.createElement('button',{type:'button',className:'account-trigger',onClick:e=>{e.stopPropagation();setOpen(v=>!v)},'aria-expanded':open},
   shownAvatar ? React.createElement('img',{src:shownAvatar,alt:'Foto de perfil',className:'account-avatar account-avatar-image'}) : React.createElement('span',{className:'account-avatar'},initials),
   React.createElement('span',{className:'account-trigger-copy'},React.createElement('small',null,'Conta pessoal'),React.createElement('b',null,displayName)),
   React.createElement(ChevronDown,{size:14,className:open?'account-chevron open':'account-chevron'})
  ),
  open && React.createElement('div',{className:'account-menu',onClick:e=>e.stopPropagation()},
   React.createElement('div',{className:'account-menu-profile'},shownAvatar ? React.createElement('img',{src:shownAvatar,alt:'Foto de perfil',className:'account-menu-avatar'}) : React.createElement('div',{className:'account-menu-avatar account-menu-initials'},initials),React.createElement('div',{className:'min-w-0'},React.createElement('strong',null,displayName),React.createElement('span',null,user?.email||''))),
   React.createElement('button',{type:'button',className:'account-menu-item',onClick:()=>inputRef.current?.click()},React.createElement('span',null,'Alterar foto'),React.createElement('small',null,'JPG, PNG ou WebP')),
   React.createElement('input',{ref:inputRef,type:'file',accept:'image/png,image/jpeg,image/webp',className:'hidden',onChange:changeAvatar}),
   React.createElement('div',{className:'account-menu-separator'}),
   React.createElement('button',{type:'button',className:'account-menu-item account-menu-logout',onClick:()=>{setOpen(false);onLogout()}},React.createElement('span',null,'Sair da conta'),React.createElement('span',null,'→'))
  )
 );
}
function TopBar({ streak, tab, setTab, exportBackup, importBackup, importExcel, theme, toggleTheme, user, onLogout }) {
 ...
 return React.createElement('aside',{className:'synapse-sidebar'},
  ...
  React.createElement('div',{className:'sidebar-bottom'},React.createElement('div',{className:'sidebar-tools'},React.createElement('div',{className:'flex items-center gap-1.5 text-sm',style:{color:streak>0?'var(--ember)':'var(--muted)'}},React.createElement(Flame,{size:16,strokeWidth:2}),React.createElement('span',null,streak,' ',streak===1?'dia':'dias')),backupButtons),React.createElement(AccountMenu,{user,onLogout}))
 );
}
