...
function SynapseWorkspace({ user, onLogout }) {
 ...
 const [theme, setTheme] = useState(() => { try { return persistentStorage?.getItem('synapse-theme') || 'dark'; } catch (e) { return 'dark'; } });
 ...
 useEffect(() => { try { if (persistentStorage) persistentStorage.setItem('synapse-theme', theme); } catch (e) {} }, [theme]);
   ...
   try {
    ...
    if (cloud.hasCloudData) {
     ...
     const [c1,c2,c3,c4,c5] = await Promise.all([
      storage.get('mindset-checkins').catch(()=>null), storage.get('mindset-clients').catch(()=>null), storage.get('mindset-reminders').catch(()=>null), storage.get('synapse-desidentificacao').catch(()=>null), storage.get('synapse-templates').catch(()=>null)
     ]);
 ...
 useEffect(() => { if(loaded) storage.set('synapse-desidentificacao',JSON.stringify({entries:desidentificationEntries,pinned:pinnedPhrase})).catch(()=>{}); },[desidentificationEntries,pinnedPhrase,loaded]);
 useEffect(() => { if(loaded) storage.set('synapse-templates',JSON.stringify(templates)).catch(()=>{}); },[templates,loaded]);
